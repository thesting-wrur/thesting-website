...
var quick_chat = jQuery.extend(quick_chat || {}, {
    ...
    spawn_private_chat: function(chat_id, username_him, state){
        ...
        string += '<div class="quick-chat-alias-container"><input class="quick-chat-alias" type="text" autocomplete="off" maxlength="20" value="'+quick_chat.user_name+'" readonly="readonly" /></div><input type="text" class="qcm"></input></div>';
...
jQuery(document).on('keypress', "input.qcm", function(e) {
...
jQuery("input.quick-chat-send-button").on('click', function(e) {
    ...
    var textarea = jQuery(this).siblings('input.qcm');
...
jQuery("div.quick-chat-smile").on('click', function() {
    var input_textarea = jQuery(this).parents('.quick-chat-container').find('.qcm');
...
if(quick_chat.user_status != 0){
    jQuery('input.qcm').on('keyup change input paste', function() {
